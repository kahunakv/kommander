...
public static partial class RaftLoggerExtensions
{
    ...
    [LoggerMessage(Level = LogLevel.Trace, Message = "[{LocalEndpoint}/{PartitionId}/{State}] Successfully completed logs from {Endpoint} Timestamp={Timestamp} CommitedIndex={Index} Time={Elapsed}ms")]
    public static partial void LogTraceSuccessfullyCompletedLogs(this ILogger<IRaft> logger, string localEndpoint, int partitionId, RaftNodeState state, string endpoint, HLCTimestamp timestamp, long index, double elapsed);
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "[{LocalEndpoint}/{PartitionId}/{State}] {Endpoint} rejected an append reporting a log through {ReportedMax}, below the progress recorded for it; its log shrank (restarted empty?), so the recorded progress is lowered and the next heartbeat re-ships from there")]
    public static partial void LogInfoRegressedFollowerFrontierLowered(this ILogger<IRaft> logger, string localEndpoint, int partitionId, RaftNodeState state, string endpoint, long reportedMax);
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "[{LocalEndpoint}/{PartitionId}/{State}] Snapshot installed on {Endpoint} at index {Index} ({Chunks} chunk(s)) — the follower is seeded and the leader's replication cursors advance to the boundary")]
