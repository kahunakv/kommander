...
public static class RaftProposalQuorumPool
{
    ...
    /// <param name="startTicks">Local monotonic registration tick; see <see cref="RaftProposalQuorum.StartTicks"/>.</param>
    /// <returns>A RaftProposalQuorum instance that is either rented from the pool or newly created.</returns>
    public static RaftProposalQuorum Rent(List<RaftLog> logs, bool autoCommit, HLCTimestamp startTimestamp, long startTicks = 0)
    {
        _pool ??= new();
        if (_pool.Count > 0)
        {
            RaftProposalQuorum rented = _pool.Pop();
            rented.Reset(logs, autoCommit, startTimestamp, startTicks);
            return rented;
        }

        return new(logs, autoCommit, startTimestamp, startTicks);
    }
