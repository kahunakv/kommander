...
public class RaftConfiguration
{
    ...
    /// <summary>
    /// Returns every safety or liveness fence this configuration leaves off or weakened; see
    /// <see cref="RaftSafetyOptionAudit"/>. Empty when every fence the audit knows about is on.
    ///
    /// <para><see cref="RaftManager"/> calls this once at startup and logs each entry, so a
    /// deployment can be audited from its own logs. It never throws and never changes a value —
    /// a value outside its valid range is <see cref="Validate"/>'s job, and that one does throw.</para>
    /// </summary>
    public IReadOnlyList<RaftSafetyOptionDeviation> GetSafetyOptionDeviations() =>
        RaftSafetyOptionAudit.Inspect(this);
