...
public sealed class TestThreeNodeClusterManyPartitions
{
    ...
    [InlineData(4, 15)]
    [InlineData(8, 21)]
    public async Task TestJoinClusterAndProposeReplicateLogsRace(int partitions, int numberOfLogs)
    {
        InMemoryCommunication communication = new();
        ...
        for (int i = 0; i < partitions; i++)
        {
            ...
            RaftReplicationResult[] responses = await Task.WhenAll(
                leader.ReplicateLogs(i, "Greeting", data),
                leader.ReplicateLogs(i, "Greeting", data),
                leader.ReplicateLogs(i, "Greeting", data)
            );
        ...
        await node1.ActorSystem.Wait();
        await node2.ActorSystem.Wait();
        await node3.ActorSystem.Wait();
        
        //Assert.True(totalFollowersReceived > 0);
        //Assert.Equal(totalLeaderReceived);
    ...
    [InlineData(4, 15)]
    [InlineData(8, 21)]
    public async Task TestJoinClusterAndProposeReplicateLogsRace2(int partitions, int numberOfLogs)
    {
        InMemoryCommunication communication = new();
    ...
    [InlineData(4, 15)]
    [InlineData(8, 21)]
    public async Task TestJoinClusterAndProposeReplicateLogsRace3(int partitions, int numberOfLogs)
    {
        InMemoryCommunication communication = new();
